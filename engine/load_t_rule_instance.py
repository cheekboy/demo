...
class view:
    ...
    def main(self):
        sqlcreate = '''
            CREATE TABLE if not exists `engine.t_rule_instance`(
              `Id` int,
              `BatchId` bigint,
              `RefElementArtfiactId` int,
              `RefElementArtifactName` string,
              `RuleName` string,
              `RuleCode` string,
              `RuleExecutedStartDatetime` timestamp,
              `RuleExecutedEndDatetime` timestamp,
              `RuleExecutedDurationInMillis` int,
              `RuleExecutedStatus` string,
        ...
        sqldroppardition = "alter table engine.t_rule_instance drop partition (daykey='%s')" % self.begdate
        H.sqlquery(sqldroppardition)
        sqladdpardition = "alter table engine.t_rule_instance add partition (daykey='%s')" % self.begdate
        H.sqlquery(sqladdpardition)

        command = "/alidata1/zs/sqoop-1.4.6-cdh5.6.0/bin/sqoop import --connect  " \
                  "jdbc:mysql://rulesplatform.mysql.rds.aliyuncs.com/rules-engine?tinyInt1isBit=false " \
                  "--username hujingxin --password YhPD89lV7Y " \
                  "--hcatalog-database engine --hcatalog-table t_rule_instance " \
                  "--hcatalog-storage-stanza 'stored as orc tblproperties (\"orc.compress\"=\"SNAPPY\")' " \
                  "--hive-overwrite  --query \"SELECT * FROM t_rule_instance WHERE BatchId>= CAST(CONCAT(CAST(UNIX_TIMESTAMP('%s 00:00:00') AS CHAR),'0000000') AS SIGNED) AND BatchId < CAST(CONCAT(CAST(UNIX_TIMESTAMP('%s 00:00:00') AS CHAR),'0000000') AS SIGNED) AND  \$CONDITIONS\"  " \
...
if __name__ == '__main__':
    ...
    logfile = os.path.join(parent_abspath, 'log', 'load_t_rule_instance.log')
    f=open(logfile,"a+")
    sys.stdout =f
    print '-------------------------------------------'
    beginTime = ctime()
