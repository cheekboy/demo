...
class view:
    ...
    def importmodule(self):
        sys.path.append(
            os.path.join(os.path.abspath(os.path.join(os.path.dirname(__file__), os.path.pardir)), 'module'))
    ...
    def main(self):
        sqlcreate = '''
            CREATE TABLE if not exists `engine.t_rule_flow_instance`(
              `Id` int,
              `BatchId` bigint,
              `ArtifactId` int,
              `ArtifactName` string,
              `ArtifactVersion` string,
              `ExecutedStartDatetime` timestamp,
              `ExecutedEndDatetime` timestamp,
              `ExecutedDurationInMillis` int,
        ...
        sqldroppardition = "alter table engine.t_rule_flow_instance drop partition (daykey='%s')" % self.begdate
        H.sqlquery(sqldroppardition)
        sqladdpardition = "alter table engine.t_rule_flow_instance add partition (daykey='%s')" % self.begdate
        ...
        command = "/alidata1/zs/sqoop-1.4.6-cdh5.6.0/bin/sqoop import --connect  " \
                  ...
                  "--hcatalog-database engine --hcatalog-table t_rule_flow_instance " \
                  "--hcatalog-storage-stanza 'stored as orc tblproperties (\"orc.compress\"=\"SNAPPY\")' " \
                  "--hive-overwrite  --query \"SELECT * FROM t_rule_flow_instance WHERE BatchId>= CAST(CONCAT(CAST(UNIX_TIMESTAMP('%s 00:00:00') AS CHAR),'0000000') AS SIGNED) AND BatchId < CAST(CONCAT(CAST(UNIX_TIMESTAMP('%s 00:00:00') AS CHAR),'0000000') AS SIGNED) AND  \$CONDITIONS\"  " \
...
if __name__ == '__main__':
    ...
    logfile = os.path.join(parent_abspath, 'log', 'load_t_rule_flow_instance.log')
    f=open(logfile,"a+")
    sys.stdout =f
    print '-------------------------------------------'
    beginTime = ctime()
